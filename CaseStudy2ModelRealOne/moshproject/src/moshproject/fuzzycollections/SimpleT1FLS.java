...
public class SimpleT1FLS {
	...
	public SimpleT1FLS() {
		...
		T1MF_Gauangle unfriendlyServiceMF = new T1MF_Gauangle("MF for unfriendly service", 0.0, 0.05, 6.0);
		...
		T1_Antecedent greatFood = new T1_Antecedent("GreatFood", greatFoodMF,
				food);

		T1_Antecedent unfriendlyService = new T1_Antecedent(
				"UnfriendlyService", unfriendlyServiceMF, service);
		T1_Antecedent okService = new T1_Antecedent("OkService", okServiceMF,
				service);
		T1_Antecedent friendlyService = new T1_Antecedent("FriendlyService",
				friendlyServiceMF, service);

		T1_Consequent lowTip = new T1_Consequent("LowTip", lowTipMF, tip);
		T1_Consequent mediumTip = new T1_Consequent("MediumTip", mediumTipMF,
				tip);
		T1_Consequent highTip = new T1_Consequent("HighTip", highTipMF, tip);

		// Set up the rulebase and add rules
		rulebase = new T1_Rulebase(6);
		rulebase.addRule(new T1_Rule(new T1_Antecedent[] { badFood,
				unfriendlyService }, lowTip));
		rulebase.addRule(new T1_Rule(
				new T1_Antecedent[] { badFood, okService }, lowTip));
		rulebase.addRule(new T1_Rule(new T1_Antecedent[] { badFood,
				friendlyService }, mediumTip));
		rulebase.addRule(new T1_Rule(new T1_Antecedent[] { greatFood,
				unfriendlyService }, lowTip));
		rulebase.addRule(new T1_Rule(
				new T1_Antecedent[] { greatFood, okService }, mediumTip));
		rulebase.addRule(new T1_Rule(new T1_Antecedent[] { greatFood,
		...
		plotMFs("Food Quality Membership Functions", new T1MF_Interface[] {
				badFoodMF, greatFoodMF }, food.getDomain(), 100);
		plotMFs("Service Level Membership Functions", new T1MF_Interface[] {
				unfriendlyServiceMF, okServiceMF, friendlyServiceMF },
				service.getDomain(), 100);
		plotMFs("Level of Tip Membership Functions", new T1MF_Interface[] {
				lowTipMF, mediumTipMF, highTipMF }, tip.getDomain(), 100);
	...
	private void getTip(double foodQuality, double serviceLevel) {
		...
		System.out
				.println("Using height defuzzification, the FLS recommends a tip of"
						+ "tip of: " + rulebase.evaluate(0).get(tip));
		System.out
				.println("Using centroid defuzzification, the FLS recommends a tip of"
